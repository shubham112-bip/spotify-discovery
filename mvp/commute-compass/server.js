  ...
  try {
    ...
    try {
      ...
      if (topArtistsRes.ok) {
        ...
        topArtistsText = topArtistsData.items.map(a => `${a.name} (${a.genres.slice(0,2).join(', ')})`).join(', ');
      }
      if (topTracksRes.ok) {
        const topTracksData = await topTracksRes.json();
        topTracksText = topTracksData.items.map(t => `${t.name} by ${t.artists.map(a => a.name).join('&')}`).join(', ');
      }
    } catch (err) {
